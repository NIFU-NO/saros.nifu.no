...
mine_data <- saros::ex_survey1 # Eksempeldata
```
...
fig_a_uni_cat_prop_plot <-  
  saros::ex_survey1 %>% # Bytt ut med eget datasett   
  rowwise() %>%  
  mutate(a_99 = sum(as.integer(c_across(matches("a_[1-9]$")))-1),
         across(matches("a_[1-9]$"), 
                ~factor(ifelse(a_99==0,
                        ...
                        levels=c("Yes", "No")))) %>%
  ungroup() %>%
  labelled::copy_labels_from(saros::ex_survey1) %>% # Må kopiere tilbake variabeletiketter som forsvinner når man bruker mutate
...
fig_c_uni_int_prop_plot <-   
  ...
  tidyr::pivot_longer(cols=matches("c_"), 
                      names_to = "variable") %>%  
  left_join(y=labelled::look_for(saros::ex_survey1, 
                                 "c_", details=F), 
            by="variable") %>%   
...
## Rotere tekst på spørsmål (fasettene helt til venstre) i bivariate figurer

Å ha teksten vertikal kan spare en del plass horisontalt om man har mange svarkategorier, men er litt vanskeligere å lese, og tar mer plass vertikalt.

```{r}
fig_b_bi_catcat_prop_plot_snudd <-
  fig_b_bi_catcat_prop_plot + # Sett teksten horisontal
  ggplot2::theme(strip.text.y.left = ggplot2::element_text(angle = 0))
ggiraph::girafe(ggobj = fig_b_bi_catcat_prop_plot_snudd)
```
